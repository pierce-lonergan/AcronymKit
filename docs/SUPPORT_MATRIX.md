...
| **CLI** `acronymkit …` | Yes | Yes, once `click` is present. `generate`, `backronym`, `synthesize`, `extract`, `score`, `tokens`, `schema`, `doctor`, `version`, and the governed half — `expand-token`, `expand-identifier`, `physical-name`, `normalize-name`, `check-name`, `governed-batch`, `governed-audit`, `governed-gap`. Every one takes `--format json`. This row previously opened with *All of* and then named nine of the then-sixteen, omitting exactly the seven the positioning led with; the CLI now declares `17` and the governed half is `8`, which is the list above — `governed-gap` is the newest and the one governed command that does not require `--dictionary`, because what it reports is derived without a vocabulary. Measured exit codes: 0 on success, 1 when the engine declines (e.g. an unavailable tier), 2 for a usage error or a missing `click` | `--tier statistical_nlp` selects it; otherwise identical | Same | `--tier neural` is accepted and warns | `pip install 'acronymkit[cli]'` — `click`, plus `colorama` on Windows. Neither carries an HTTP client |
