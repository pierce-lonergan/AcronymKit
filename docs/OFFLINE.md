...
seven data files that ship inside the distribution, and the JSON Schema used for interchange validation
is read from that same bundle rather than from the filesystem. Nothing downloads a model, phones home,
or reads a user's home directory. What does *not* work offline is the neural tier, which is not
...
public API and every CLI subcommand inside a routeless network namespace, with a positive control that
...
**Bytecode scan.** All 27 shipped modules under `src/acronymkit/` were compiled and every code object
...
index; the full suite under the guard above; and the whole public API plus every CLI subcommand inside
...
- `acronymkit.resources.resource_path()` calls `importlib.resources.as_file`, which materialises a
  temporary file when the package is loaded from a zip. Calling it over all seven bundled resources on
  an ordinary unzipped install produced **0** temp-file and **0** write events; it returned the real
  resource path each time. The temporary files, if a zip install ever creates them, are released by an
...
Seven data files, and nothing else, ship inside `acronymkit/resources/`. Digests below are from
...
Two things the table implies, stated rather than left to be inferred:

- **Only English ships a lexicon and an n-gram model.** The French, Spanish and German Hunspell
  dictionaries used during development are copyleft, are marked fetch-only in `data/LICENSES.md`, are
  git-ignored, and are not in the distribution. `tools/build_lexicons.py` refuses to vendor anything
  marked fetch-only, so the split is enforced by code rather than by diligence.
- **Nothing in the distribution is downloaded at runtime, and no resource is fetched on demand.** The
  seven files above are the complete data surface. `data/LICENSES.md` is the full asset ledger,
...
is 205,920 B — 53.2 % of the compressed total. This is the artifact from the last build on this tree;
see section 9 on why an artifact figure has to be re-derived per release.
