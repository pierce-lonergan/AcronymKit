...
arrive is scoped by the same people it grades"* has been a number rather than a defence.
**The seventh sweep leaves the count at `11` of `20` and that is the least informative headline this
page could print, because two criteria swapped places underneath it.** Criterion `15` — R16, no figure
without a regenerating script and a CI diff — **closed by being done**, and it closed the right way
round: the script, the gate and the CI step were built and then the first figures shipped through them.
Criterion `7` — *no known defect where a report claims clean while data is lost* — **opened**, because
the round that fixed a fourth defect of exactly that class found a fifth in the same subsystem and did
not fix it. `7` had been carried as met, un-re-derived, for four consecutive sweeps. **A criterion that
survives four sweeps by being carried and fails the first sweep that re-derives it is the argument for
re-deriving, made by this page against itself.** They have been carried in mandates and answered one at a time in `docs/DECISIONS.md`; they had
...
**The eighth sweep moved no verdict at all and the ninth moves exactly one, in the direction that
does not look like progress.** The count stays at `11` of `20`. Criterion `13` — the deferred ledger's
policy and measured trajectory — **closes by narrowing**: it is still met as written, because a
written policy and a measured trajectory both exist, and the ninth sweep established that the
trajectory has been measuring the ledger that cannot move. `docs/DECISIONS.md` carries `42` deferred
numbers and, separately, `42` value-matched ones; three independent walks all resolved the first
population and measured it terminal; `13` of the second are unambiguously citable today. The quota
reads only the first. **That is a criterion whose verdict is unchanged and whose meaning shrank, and
the verdict column says so rather than leaving the row reading as it did.** D-126.

**Four rows had their evidence corrected this sweep without their verdicts moving**, which is the
other thing this page exists to do. Criterion `3` records two new ungated figures the gate cannot
see; criterion `9` records `21` of `42` and the round in which the register was wrong about its own
arithmetic because a workstream measured it on a stale checkout; criterion `14` records that two of
its three closing clauses had gone stale — `gates.second_reader` **is** registered and **is** in CI —
and that a fourth consecutive cold read wrote no ledger row; criterion `18` records that the largest
byte-identity result this project has produced came out of a script `git ls-files` does not match.

**Re-derivation, ninth sweep: five of twenty were re-derived by running the check** — `3`, `9`, `13`,
`14` and `18`, each against the reconciled tree described in D-119 — and fifteen were carried.
`1`, `2`, `5`, `6` and `7` have now been carried for six sweeps running. **The one verdict this
sweep could not check at all is the byte-identity result underwriting criterion `18`**: the governed
corpora are absent from this checkout and no harness is committed. Unmeasurable here, and that is
recorded as the answer rather than resolved into a `met`.

**The tenth sweep moves exactly one verdict too, and it is criterion `13` for a second consecutive
sweep — this time because something was PAID rather than because something was re-read.** The count
stays at `11` of `20`, for the fourth sweep running. The ninth sweep established that the trajectory
had been measuring the ledger that cannot move; the tenth found that the diagnosis needed one
correction of scope. **`MIGRATION_QUOTA` is a floor on the fall over the whole deferred ledger and
only `RECORD_FILE_FLOOR` is about `docs/DECISIONS.md`** — so four consecutive walks resolved one file
of ten and then wrote the waiver against a quota that reads all ten, leaving `147` of `189` numbers
unprobed by anybody. The tenth walk is the first over all ten files, it paid `4` of a payable
population of `4`, and the ledger fell `189` to `185` for the first time since `201`. **The verdict
is still `met` and its meaning moved again, in the opposite direction from the ninth sweep's.**
D-136.

**Six rows had their evidence corrected this sweep without their verdicts moving.** Criterion `1`
records a second refusal mechanism that now carries a *selective* guarantee — the quantity D-104
showed the conformal gate does not bound — holding in `8` of `18` cells and refusing outright on the
corpus this criterion is scored on. Criterion `3` records the ledger's first fall since `201`.
Criterion `7` records a **third** live instance, found by exhaustive enumeration rather than by a
report: `26` code points for which `to_physical_name` says `unaccounted=()` and then drops a
combining mark. Criterion `9` records `21` of `42` unchanged and four workstreams that each had a
gate to register, each declined, and each argued the refusal from the same ceiling. Criterion `14`
records a fifth consecutive cold read with no ledger row **and the mechanism for it**: the anti-rot
clock counts ledger rows rather than reads, and the only agent that could write one is forbidden to.
Criterion `18` records the R19 rule **catching a real defect for the first time in its history**,
`3` of `20,000`, and `0` on both real corpora because both run an empty catalog.

**Re-derivation, tenth sweep: seven of twenty were re-derived by running the check** — `3`, `7`, `9`,
`13`, `14`, `17` and `18` — and thirteen were carried. `2`, `5` and `6` have now been carried for
seven sweeps running. **The one verdict this sweep could not check is the same one the ninth could
not**: the `3,619,227`-record byte-identity result underwriting criterion `18` is still not
reproducible here and `git ls-files` still matches no snapshot harness. Unmeasurable here, and that
is recorded as the answer. Criterion `19` (W11) is `not started` for a fourth consecutive sweep and
was assigned to nobody again.

**The eleventh sweep moves nothing at all. The eighth also moved nothing, so the count is not new;
what is new is that this time it is the correct answer rather than a symptom.** The count stays at `11` of `20`, for the fifth sweep running. This sweep ran against
the `0.4.0` release-preparation tree, where **no party changed a line of `src/`**: the whole round's
diff is `CHANGELOG.md` and one line of `pyproject.toml`. A definition of done that moved on that round
would be grading the release notes rather than the library, so `0` is the verdict and the reason is
recorded beside it. **What a release does buy is a second vantage point on criteria that had only ever
been measured inside the checkout**, and every one of this sweep's six evidence corrections comes from
there: what the *sdist* can re-derive, which defects *ship*, and what a released changelog section does
to a live citation. D-143.

**Six rows had their evidence corrected this sweep without their verdicts moving.** Criterion `3`
records that the sdist ships `bench/results.json` and `bench/splits.toml` and **no bench runner**, so
*re-derivable offline from the sdist* is true of the figures and false of the measurements behind them
— and that the `[0.4.0]` section carries `53` rendered citations where all three previously released
sections carry `0` between them. Criterion `7` records that **two of its three live instances ship in
`0.4.0`** and that the release notes disclose one of the two. Criterion `9` records the count
MOVING, `21` to `23` of `42`, for the first time in three sweeps — and **not one gate was
demonstrated by this round on purpose**: one payment was six days old and unread, the other was an
accident. `docs/GATES.md:28`'s *"forty-one gates"* against a manifest of `42` is **fixed at the
release commit** along with the self-contradiction two screens below it, and then the corrected
sentence went stale again within the same round when the count moved. Criterion
`13` records the eleventh walk measuring the payable population at `0`. Criterion `14` records a sixth
consecutive cold read with no ledger row, and the first read observed routing around
`check_claims.SCAN_GLOBS` deliberately and saying why. Criterion `15` records the rule's edge: it keeps
figures honest and has nothing to say about citations inside a *frozen* document, where staying current
is the defect.

**Re-derivation, eleventh sweep: nine of twenty were re-derived by running the check** — `3`, `4`, `7`,
`9`, `13`, `14`, `15`, `17` and `20` — and eleven were carried, which is the highest re-derivation
count since the seventh sweep's ten (the eighth did seven, the ninth five, the tenth seven). `2`, `5` and `6` have now been carried for eight sweeps running.
**The one verdict this sweep could not check is the same one the ninth and tenth could not**, and the
parties unable to check it are now three: the `3,619,227`-record byte-identity result underwriting
criterion `18` is still not reproducible here. Criterion `19` (W11) is `not started` for a fifth
consecutive sweep and was assigned to nobody again.
...
**Seventh sweep, `2026-09-08`, after eight workstreams, a sampled-verification pass and a fifth cold
read.** Ten of twenty were re-derived by running the check or the code in the row — `1`, `3`, `7`, `9`,
`13`, `14`, `15`, `17`, `18` and `20` — and ten were carried: `2`, `4`, `5`, `6`, `8`, `10`, `11`, `12`,
`16` and `19`. **Five of the ten carried (`2`, `5`, `6`, `8`, `12`) have now been carried for five
sweeps running**, and `7` — which was in that group until this sweep — is why that sentence is no longer
a formality. Two verdicts moved, in opposite directions; one moved from *not started* to *partly met*;
and four rows whose verdicts did not move had their evidence change materially. D-098 through D-109.

**Eighth sweep, `2026-09-09`, after three workstreams, a sixth cold read and a fifth
sampled-verification pass. No verdict moved.** Seven of twenty were re-derived by running the check or
the code in the row — `3`, `9`, `14`, `15`, `17`, `18` and `19` — and thirteen were carried. **The
round's brief forecast that criterion `19` would close and it did not**, which is the most useful thing
this sweep produced: A2 shipped, and A2 is a separate opt-in module that leaves `extract()`
byte-identical, so the W11 question — *may `extract()` emit a short form with an absent or
low-confidence long form* — is **cheaper to answer and still unanswered.** A criterion is not closed by
building the thing next to it. **Criterion `15` improved twice and neither improvement was work on
criterion `15`**: its gate acquired in-situ evidence from a CI run read a fortnight late, and
`MANIFEST.in` began shipping the figures after a build job went red in an extracted tree. That is a
criterion **closing by narrowing what was open**, and the verdict column says so in those words rather
than reporting two items gone. D-110 through D-118.
...
|---|---|---|---|---|
| 1 | The disambiguator can say "I don't know" — abstention exposed, curve published decomposed, **policy confirmed on held-out data** | **met with qualification** — mechanism and publication, not value; **re-derived at the seventh sweep, verdict unchanged and the evidence underneath it moved a long way** | `min_margin`, `margin`, `abstained` ship; `disambiguation.sdu21.abstention_curve`; `docs/EVALUATION.md`; **`src/acronymkit/core/conformal.py` now ships a second refusal mechanism with a distribution-free guarantee, off by default, byte-identical when unused** (`conformal.*`, D-104) | An on-by-default `min_margin` proposal with a threshold and a cost model, then D-043's single spend of SDU-21 AD `test.json`. **The cost model now exists and it argues against the proposal**: the trivial baseline beats the gated system at every measured coverage from the reference gate rightward, and at a matched answer rate the conformal gate is a wash against the margin gate — with the margin gate given the advantage of picking its threshold on the evaluation set. **Closing this criterion got harder this round, not easier, and that is a result rather than a delay.** D-104 **At the tenth sweep the refusal mechanism acquired a SELECTIVE guarantee and the reach of it was measured rather than described.** `acronymkit.core.selective` bounds the error rate *among answers* by Learn-Then-Test, which is the quantity D-104 showed the conformal gate does not bound — and it holds in `8` of `18` (arm x alpha) cells, with all `10` cells that fail answering `0.00` %. On SDU-21 AD dev it certifies `0` of `21` thresholds at all six alphas, because the lowest selective risk reachable there is `17.53` % over a base disambiguator that is `40.82` % accurate answering everything. **So the mechanism that would let this criterion mean "I don't know" with a number attached refuses on the corpus this criterion is scored on.** Off by default, byte-identical when unused, and it did not tighten `gate=`. D-128. |
...
| 3 | Every number gated, cited by run id, re-derivable offline from the sdist; **zero un-gated figures in user-facing prose** | **not met — and the evidence got worse, correctly**; ~~re-derived at the fifth sweep~~ **CORRECTED `2026-08-26`: the sixth sweep re-derived this row too and did not update its evidence cell, which is why the cell below was two rounds stale** | `tools/check_claims.py --residue` now counts a deferred ledger and an unexamined residue that no earlier run had ever reported; two of the three named coverage gaps are closed; ~~the ledger has fallen `316` to `262` to `231` to `213` to `201` across five recorded rounds~~ — **live at the seventh round: `316` to `262` to `231` to `213` to `201` to `189` to `189`, the last of those a waiver rather than a payment (D-097)** | Migrate the deferred ledger, lowering the baseline in the same commit; widen the scan to `tools/` and `bench/`; widen the unit vocabulary — each one its own measurement. **At the eighth round the ledger stood still for a second time and the reason is now measured rather than asserted**: `70` of the `189` are terminal by construction — `42` in `docs/DECISIONS.md`, re-probed against a `bench/results.json` `3,771` numeric leaves larger and still unreachable, and `28` in `CHANGELOG.md`, refused because citing a *released* changelog entry re-points a frozen historical statement at a live value that `--render` rewrites, and three of them would be rewritten today. **A whole un-gated channel closed in the same round**: R16 means a number can no longer reach a reader through a drawing. D-109, D-102. **At the eighth round one item leaves this close-list and nothing else moves.** *Widen the unit vocabulary* is partly done: `latency` and `duration` are metric keywords and the spelled-out sub-second units are units, and the widening changed the arming class of `0` of `2246` claim-shaped numbers across the `72` scanned files — re-derived twice by the workstream and once independently. **Zero firings is the measurement and it calibrates nothing**, and the closure reaches two keywords and three units rather than a class. *Widen the scan to `tools/` and `bench/`* is untouched, and the ledger stood still for a THIRD consecutive round on a third waiver. D-112, D-118 **At the ninth sweep the evidence got worse again, in the newest direction available.** The package split rests on two tokenizer-divergence figures, `14.6560` % and `36.0072` %, and they are quoted in three NEW user-facing places with no run id behind any of them. They live in fenced blocks `tools/check_claims.py` cannot read, so the gate is silent on all three and the ratchet did not move. **The workstream said so in all three places rather than laundering them through a code span**, which is the honest form of an ungated figure and is still an ungated figure. D-120. **At the tenth sweep the ledger fell for the first time since `201`, by `4`, and the number that matters is the denominator behind it.** `316` to `262` to `231` to `213` to `201` to `189` four times over to `185`. The `4` are real citations with named run ids — two of them in a sentence that already named `governed_gold.socrata.gold_conflict` in prose and lacked only the machine-readable form. **And the walk that found them is the first over all ten files**: four previous walks resolved `docs/DECISIONS.md` and nothing else, so `147` of the `189` had never been probed. The payable population across the whole ledger is `4`, and `0` of `docs/DECISIONS.md`'s `42`. *Widen the scan to `tools/` and `bench/`* is still untouched. D-136. **At the eleventh sweep the sdist half is measured rather than assumed.** The `0.4.0` sdist ships `bench/results.json` and `bench/splits.toml` and **no bench runner**, while all `25` tools ship and `render_figures.py --check` passes inside the extracted artifact: *re-derivable offline from the sdist* is therefore true of the figures and false of the measurements behind them, which is narrower than the criterion reads. **And a hazard the criterion did not anticipate:** the `[0.4.0]` section carries `53` rendered `<!--claim:-->` citations where `[0.3.0]`, `[0.2.0]` and `[0.1.0]` carry `0` between them, so cutting the version turns all `53` into live values that the next `--render` over a moved field would rewrite inside a released release note. `--render --dry-run` reports nothing would change today, and that is the whole of the protection. D-143. |
...
| 7 | No known defect where a report claims clean while data is lost | **NOT met — re-opened at the seventh sweep, on a live instance, after four sweeps of being carried** | The three governed defects of D-024 are still fixed. **A fourth of the same class was live in the same subsystem the whole time and is fixed this round** — `normalize` dropped unaccounted characters and returned the shortened name, `is_compliant` offered a `fix` that deleted the character it could not read, and `to_physical_name` reported `confidence` `1.0` on a name it had shortened (D-099, `857,517` records compared, `112,494` moved, `0` moved without exercising the defect). **A fifth is open and unfixed**: `loaders._read_pairs` discards CSV rows with no count and returns a `GovernedDictionary` with no member anywhere saying rows were dropped — a `5`-row file with two unusable rows returns `3` entries and reports clean | Fix `loaders._read_pairs`, which is a return-type change and therefore a breaking change owing its own byte-identity pass. **And stop carrying this row.** It survived four sweeps as *met* by being carried, and the first sweep that re-derived it found two instances, one of them shipped for the whole of that period. "Known" is the whole content of the claim and nobody was looking. **At the tenth sweep a THIRD live instance arrived, found by exhaustive enumeration rather than by a report**: for `26` code points `to_physical_name` reports `unaccounted=()` on the first pass and drops a combining mark on the second, so it emits a physical name `normalize` refuses to read back — `26` of `26`, re-derived independently by the recorder over all `1,114,112` code points. **The two halves of the governed round trip disagree about these inputs and one of them says it lost nothing.** Incidence is `0` across the `285,839` distinct strings in `data/governed_gold/`, which is an argument that a repair would move nothing and not a demonstration of it. Recorded as a strict `xfail` with a positive pin beside it, not fixed. **A criterion at three live instances is further from met than when it re-opened.** D-130. **At the eleventh sweep, still NOT met, and the release makes the cost concrete: two of the three live instances SHIP in `0.4.0`.** The `26`-code-point round trip is disclosed in the release notes' KNOWN-OPEN block; the `loaders._read_pairs` row-dropping instance is disclosed nowhere a reader of the release will look. Re-derived this sweep in a fresh process: a five-data-row CSV with one blank-key and one blank-value row returns `3` entries, `0` warnings and no member on the returned `GovernedDictionary` naming a drop. **The shallow instance is the one the notes omit and the deep one is the one they carry, which is the wrong way round for a first-time caller.** D-139, D-143. |
...
| 9 | **NEW (Mandate II).** Every CI gate has a recorded in-situ mutation test, stored as an artifact | **not met — and at the sixth sweep the count moved DOWN, `13` of `36` to `12` of `36`, the first fall this page has recorded**; re-derived at the fifth and sixth sweeps | `.github/gates.toml`, `tools/gates.py --check`, `docs/GATES.md`; D-061, D-079. The register now prints ~~`CARRYING IN-SITU EVIDENCE: 13 of 36`~~ **`CARRYING IN-SITU EVIDENCE: 12 of 36`, corrected `2026-08-26` — D-090 recorded amending this row's `13 of 36` and amended the verdict cell only, leaving this cell contradicting the verdict beside it for a round**, a `cost_rank` on every gate with the top `3` demonstrated, and a per-round quota stated as a **ceiling on the debt** so that adding gates cannot dilute the count | A push per remaining gate. The thirteen are **every gate this harness could ever mutate**; the other `23` are `8` inline, `13` manual and `2` control refusals. Three of the eight inline refusals fall to one afternoon of extracting heredocs into `tools/`. **That afternoon happened and the costing was wrong about one of the three**: `inline` is now `5`, automated `16`, debt `24` at a ceiling of `24`, `4` gates owed forward, and one gate's evidence withdrawn because its recorded verdict was reachable with the declared defect uncaught. D-087. **At the seventh sweep the count rose to `16 of 38`, debt `22` at a ceiling of `22`, `2` owed forward — re-derived by the recorder running `python tools/gates.py --check` — and the four gates that paid the debt had been demonstrated on a runner a fortnight earlier and nobody had read the log.** Two gates were registered (`figures`, `memo_identity`), and a rule this register called absolute became waivable for the second time in three rounds, because a correctly-ranked new gate cannot carry evidence in the commit that creates it. D-106. **At the eighth sweep `16 of 38` becomes `18 of 39`, re-derived by the recorder running `python tools/gates.py --check`, and the mechanism is D-106's for the second time in three rounds**: `figures` and `memo_identity` had been demonstrated in Actions run `34308556192` at `4ef57c8` two days earlier and the register printed a lower number until somebody read the log. A thirty-ninth gate was registered (`gates.run_summary`, `cost_rank 37`) and **owes its own in-situ evidence forward**, because the workstream that registered it cannot push. Nothing installed this round makes the next round read the log. D-113 **At the ninth sweep the count rose to `21` of `42`, and for one round the register was wrong about its own arithmetic.** Two gates were added: `gates.agent_summary` was demonstrated IN SITU in the same session at run `34408932625` on `5a268cd`, read from the artifact rather than inferred from a tick, and `gates.architecture_boundaries` **owes its evidence forward** because the workstream that registered it cannot push. Debt `21` at a ceiling of `21`. **The register printed `in_situ=20` against a tree carrying `21` for the length of the round**, because the row was measured on a checkout three commits behind `origin/main`; the validator caught it, the recorder corrected the integer, and D-119 is the account. `50` % of gates now carry evidence, and it has taken ten rounds to get there. D-119, D-120, D-121. **At the tenth sweep nothing moved and the reason is now a pattern rather than an incident**: `21` of `42`, debt `21` at a ceiling of `21`, `1` owed forward, re-derived by the recorder running `python tools/gates.py --check`. **Four workstreams each had a gate to register and all four declined, and all four argued the refusal from the same fact** — the debt is at its ceiling, so a forty-third gate needs a waiver whose only available text is *"this workstream cannot push"*, which D-119 records being printed for a fortnight. Two of them offered `gates.suite` as the enforcement surface instead and one rewrote its `detects`/`blind_to` rather than adding a row. **The ceiling is doing what it was designed to do and the side effect is that the register stops growing where the evidence is**: the strongest correctness statement added this round — an exhaustive walk of the whole code space — has no register row at all. D-129, D-130. **At the eleventh sweep `21` of `42` is unchanged and no party registered a gate, correctly — nothing new was built to gate.** At the eleventh sweep, `docs/GATES.md:28` published *"forty-one gates"* where `tools/gates.py --check` printed `42`, so the page describing the register disagreed with the register by one, in a figure spelled as a word that no gate can read (D-143) — **and worse, that page's own section on the forty-second gate said "ranked `40` of `42`" two screens below, so it contradicted itself rather than merely lagging. FIXED at the `0.4.0` release commit**, with the contradiction named on the page so the next reader knows the count has been wrong twice by exactly one round. **Nothing gates it**: the figure is spelled as a word, `prose_of()` masks the code span beside it, and the claims gate is provably green over both. **And then the count itself moved, `21` to `23` of `42`, in the same commit** — `gates.architecture_boundaries` had been demonstrated in Actions run `34418093244` on `1e0d6d5` SIX DAYS earlier, on the very commit whose waiver predicted that run would pay it, and went unrecorded through two further green demonstrations; and `gates.installed_expected_non_passing` became the first `manual` gate to carry in-situ evidence by firing for real on a defect this round wrote. **Fifth instance of the unread-log pattern**, after D-106, D-113, D-119 and this row's own ninth-sweep entry. Debt `21` to `19` at a ceiling of `19`; quota waived at `2` of `3` because the third payment would have been an inline extraction on a release commit. **The verdict does not move: `23` of `42` is still not every gate**, and `19` gates carrying no in-situ evidence is what the criterion is about. D-145. |
...
| 13 | **NEW (Mandate II).** The deferred ledger has a written policy and a measured trajectory | **met, and NARROWED at the ninth sweep — the trajectory exists and has been measuring the ledger that cannot move**; four observations became ten, the fall stopped four rounds ago, and the criterion closes by narrowing rather than by progress; **MOVED AGAIN at the tenth sweep, in the opposite direction — eleven observations, the first non-zero fall in five rounds, and the ninth sweep's narrowing is now measured over all ten files instead of inferred from one** | `docs/CLAIMS-LEDGER.md`; `LEDGER_TRAJECTORY`, `MIGRATION_QUOTA = 12`, `RECORD_FILE_FLOOR = 12`, `RECORD_FILE_PIN` and `trajectory_problems()` in `tools/check_claims.py`, mutation-tested red; D-059, D-071 | Nothing for the literal criterion. Four migrations of `54`, `31`, `18` and `12` are a rate and it is decelerating. `docs/DECISIONS.md` is down from `115` to `54` and the recorder is bound by a per-file floor. **The forecast that the `66` remaining held no citable measurement was wrong**: the walk found `3` citations and `9` deletions and paid the floor exactly, because *blocked* had again been a per-record verdict applied per number. `54` remain and the *next* round owes a waiver naming three mechanisms; D-084. **At the sixth sweep the predicted waiver did not arrive for a second consecutive round**: `12` more came out by `3` citations and `9` deletions, `54` → `42`, and *blocked* had again been a per-record verdict applied per number — this time for a reason that can never be discharged. D-091. **At the seventh round the waiver finally arrived, and it is the first this project has written.** `0` migrated, `42` remaining, and the reason is a measurement rather than an opinion: all `42` were resolved against every field in `bench/results.json` and `26` match nothing anywhere while the other `16` match only unrelated quantities in unrelated units. Deletion was walked and refused three times with reasons. **Read the row as a waiver at the end of a burn-down and not one taken to avoid one — the probe is the only thing separating those two objects, and the next round owes the same probe.** D-097. **At the eighth round the probe was run as owed and returned a different answer in one cell, and the round took a SECOND consecutive waiver.** `bench/results.json` grew `9.89` percent in numeric leaves during the phase while the coincidental field matches for the same forty-two sentences grew `19.49` percent, and one number crossed from matching nothing anywhere to matching a conformal coverage percentage — **value-matched backing degrades faster than the measurement corpus grows.** The round then tried to pay out of `CHANGELOG.md` and refused on a measurement, so the reachable register is `70` smaller than the trajectory implies. **Two consecutive waivers is this policy reporting its terminal state, and D-109 escalates three specific replacements to the maintainer rather than writing a third.** D-109 **At the ninth sweep the verdict narrows, and the reason is an instrument defect rather than a fourth walk of the same residue.** The trajectory now has ten observations and has stood still for four consecutive rounds on four consecutive waivers. The residue WAS re-walked rather than inherited and it reproduces terminal for a third independent time — `25` of the `42` in `docs/DECISIONS.md` match nothing anywhere, `17` coincide in the wrong unit, against D-109's `26`/`16`, the split having moved by one in the direction D-109 predicted. **The new fact is that the trajectory has been measuring the ledger that cannot move.** That file carries `42` deferred numbers AND, separately, `42` value-matched ones — the coincidence of totals is why three walks all resolved the same population — and `13` of the second `42` are unambiguously citable today with named run ids. `fall` is computed on the deferred ledger alone, so **a row recording those `13` honestly turns the gate red while recording them as zero passes clean.** The criterion is still met as written — a written policy and a measured trajectory both exist — and what the trajectory measures is narrower than the criterion reads. D-126. **At the tenth sweep the trajectory recorded a movement for the first time in five rounds and the ninth sweep's diagnosis needs one correction.** `MIGRATION_QUOTA` is a floor on the fall over the **whole** deferred ledger; only `RECORD_FILE_FLOOR` is about `docs/DECISIONS.md`. Every one of the four previous walks resolved that one file and then wrote the waiver against a quota that reads ten, so `147` of `189` numbers had never been probed by anybody — including the `70` in `docs/notes/pydantic-cost.md` that D-126 named as never probed and assigned to nobody. **The tenth walk is the first over all ten files and it pays `4` of a payable population of `4`**: `189` to `185`, `by_citation=4`, `from_record_file=0`. Two probes were run — `--classify` (gate-able `56`, blocked `129`, not-a-claim `4`; the `56` resolving to `7` UNIQUE, `0` REPLICATED, `49` AMBIGUOUS) and a new run-id-naming probe restricting candidates to fields under a run id the surrounding prose names (`9` CITABLE, `6` of them in whole-file-blocked `CHANGELOG.md`). **The record file is `0` of `42`, and this is the first walk to NAME the five that looked payable and show each one to be a millisecond or microsecond matched against a percentage** rather than to report a total. **So the fifth waiver stands and it is required by arithmetic**: a floor of `12` per round on a measured population of `4` cannot be satisfied by any round, which is a different statement from *the residue is terminal* and needs a different fix. D-136. **At the eleventh sweep the walk was re-run over all ten files rather than inherited and the payable population is `0`.** `--classify` puts the `185` at gate-able `52`, blocked `129`, not-a-claim `4`; the run-id-naming probe returns CITABLE `6`, and **all `6` are in `CHANGELOG.md`** — whole-file blocked since D-109, owned by another workstream this round, and frozen for a cold read of the release. `gate-able` fell `56` to `52` and CITABLE `9` to `6`, both by exactly D-136's four payments, so the probe reproduces. Sixth waiver, `fall` `0`, arithmetic rather than judgement. D-144. |
| 14 | **NEW (Mandate II).** A second reader exists, in some form, for anything user-facing | **met, and the criterion is less modest and more exposed than it was**; re-derived at the fifth sweep | `docs/SECOND-READER.md` — two triggers, six checks, costed at one agent slot; wired into `CONTRIBUTING.md`; first pass found `15` defects, `6` material, `7` of `15` in files the round never touched; a second pass has now run **as policy**, shipped six fixes, carried the rotation cursor in §8 and found three defects in the policy itself; the trigger is now executable code with a test, the hand-off is a validated ledger, and `disposition = "fixed"` requires a second name; D-060, D-072, D-081 | Nothing for the literal criterion. Nothing enforces it: the enforcing CI job is specified in one paragraph and not built. **And the third read wrote no ledger row**, because the read-only instruction and the ledger requirement could not both be obeyed — so the cursor did not advance and the next reader serves the file the third reader just served. **At the sixth sweep the two-round survival of a retired sentence in `src/acronymkit/__init__.py` acquired a mechanical answer rather than an attribution**: `PATHSPEC` has `6` entries with none under `src/`, `user_facing_files()` returns `21` paths with `0` under `src/`, and in situ the trigger returns four files and not the source file the round had just changed. D-089. **A fourth read has now run and written no ledger row either — the second consecutive time**, so the cursor has not advanced for two reads, the rotation served the same file twice, and `11` new findings live only in a note that no rule in the policy reaches. `python tools/second_reader.py --check` is green throughout, **and it is green precisely because no row was written**: the gate cannot tell *no cold read happened* from *a cold read happened and could not record it*. The blocking decision is one sentence nobody has taken — whether the read-only boundary is prose or filesystem — and §5.1 says in terms that it is not a filesystem permission. D-096. **A fifth read has now run and written no ledger row either, so two of five reads have none and the cursor has stalled for a second consecutive read on the same file.** Its structural finding is that `is_user_facing()` drops every non-Markdown file under `docs/`, so **the four figures this round created are outside both triggers, outside the rotation and outside `--check`'s tree-reachability rule** — their numbers are gated twice and their title sentence, shading and accent assignment by nothing. It also found the round's largest claim **unadjudicable in a checkout with no GitHub token**, which is a class of claim no round has priced. **Three of that read's eight findings were then applied to the tree by an operator, and the ledger row was written by a second party, both after D-108 was written and both during the recorder's verification pass**: `is_user_facing` admits `docs/figures/*.svg`, the rotation is `25` files, trigger A now serves `13`, `docs/GATES.md`'s tracked-file count and two published commands are corrected, the cursor has moved to `docs/SECOND-READER.md`, and `docs/cold-reads.toml` now records `4` reads and `15` findings — `10` fixed, `5` open, each open one carrying a `blocked_on`. **So the row that two consecutive rounds could not write was written by somebody who was not the reader, which is what §5's disposition rule was asking for all along.** What the round still demonstrated is the gate blind in the other direction: the same `--check` printed `open 0, fixed 7` over a tree in which three of those repairs had already landed, and it printed `rc=0`. It cannot see a read that could not record itself, and it cannot see a repair that recorded nothing. D-108. **A sixth read has now run, written no ledger row, and stalled the cursor for a third consecutive read** — `--check` prints `cold reads: 4 recorded` over six that happened, and `trigger B serves docs/SECOND-READER.md next` over a read that already served it. Its own headline finding was reached by grepping the tree rather than by either trigger, which is the **fourth consecutive read** to find its main defect outside the rotation. And it found that `tools/second_reader.py --check` — the eighth gate every brief names — **is in no CI job and no `.github/gates.toml` entry**, so the gate that audits the cold reads is not among the `39` gates the register counts. D-116 **At the ninth sweep two of that sentence's three clauses are stale and the third one held.** `gates.second_reader` IS registered — `.github/gates.toml:2378`, the fortieth gate — and `ci.yml:129` HAS run `python tools/second_reader.py --check` since `fbf7c45`, so the gate that audits the cold reads is among the `42` the register counts. It still has no section in `docs/GATES.md`, named in that page's header by a round that did not build it. **The sixth cold read wrote ten findings and no ledger row, for the fourth consecutive read**: `--check` prints `4 recorded` while six reads have run, and all five open findings reach the two-read limit at the next read, so `OPEN AND AT THE LIMIT` goes `0 of 5` to `5 of 5`. Eight of the ten findings stand on the reconciled tree; the two that fell are the two that turned on ABSENCE, which a checkout three commits stale manufactures. D-119, D-124. **At the tenth sweep a seventh read has run, written no ledger row for the FIFTH consecutive read, and diagnosed the mechanism.** `--check` prints `cold reads: 4 recorded` over seven that have happened, `OPEN AND AT THE LIMIT: 0 of 5` over five findings whose `reviewed_in` still says read five, and `rc=0` throughout. `F-7-07`: **the anti-rot clock counts ledger ROWS, not reads** — both the cursor check and `OPEN_READ_LIMIT` count rows in `docs/cold-reads.toml`, and the only agent holding the information to write one is forbidden by §`5` from writing it, so the clock cannot advance while the policy is obeyed. Trigger B served `docs/SECOND-READER.md` for the second consecutive read and the four SVG figures in the rotation went unread again. **Ten findings written, zero applied — fourteen defects across two rounds with none repaired, and four of this read's ten are re-affirmations of read six's.** D-133. **At the eleventh sweep a sixth consecutive cold read wrote no ledger row, and this one names the mechanism from the inside**: it placed its findings outside `docs/notes/` *because* that directory is in `check_claims.SCAN_GLOBS` and a new file of figures there would push the deferred ledger past `185` and redden the gate. The anti-rot clock counts ledger rows; the only agent that could write one is forbidden to; a read has now been observed routing around the instrument correctly and saying why. D-142, D-143. |
| 15 | **NEW (Mandate III).** No figure ships in a document unless a committed script regenerates it from `bench/results.json` and CI diffs it (R16) | **met at the seventh sweep, and it closed the right way round** — the script, the gate and the CI step were built and the first figures then shipped through them, which is the order the rule was written to force. ~~not met, not started — and there is no live instance to be not-started about~~ | `tools/render_figures.py` (`--check` re-renders and diffs bytes), `tests/test_render_figures.py` (`67` tests), `docs/figures/` (`2` figures x `2` themes), `gates.figures` registered at `cost_rank = 3` with a `lint` step in `ci.yml`. A `Measurements` object is the only path from `bench/results.json` into a drawing, citation resolution is **imported from `check_claims`** so a dead run id fails identically in an SVG and in prose, and `TestNoNumberOnACanvasIsUncited` reads every numeric token back off every `<text>` element and refuses any the `<desc>` does not cite. Demonstrated failing four ways locally and once independently by the recorder, with a byte-verified restore. D-102 | Nothing for the literal criterion. **Three things are open and none of them is the rule.** The gate has never been demonstrated in situ — `top of the cost ranking: 2 of 3 demonstrated`, and `figures` is the third. `MANIFEST.in` does not ship `docs/figures/*.svg`, so half the R16 test cannot run in an extracted sdist. And **no shipped document links the figures**, so the mechanism currently defends a directory rather than a page. The allowlist of six axis ticks is a real hole and nothing gates legibility. **CLOSING BY NARROWING, AND SAID SO IN THIS COLUMN RATHER THAN IN A FOOTNOTE.** Two of those three are gone at the eighth sweep and **neither closure was work on this criterion**: `gates.figures` carries in-situ evidence (`top of the cost ranking: 3 of 3 demonstrated`) from a CI run nobody read for a fortnight, and `MANIFEST.in` ships `docs/figures/*.svg` because a build job went red in an extracted tree that could not find them. The third is unchanged — **no shipped document links a figure**, verified this sweep — so the mechanism still defends a directory. D-113, D-117 **At the eleventh sweep, still met, and the release shows the rule's edge.** The mechanism keeps `docs/figures/*.svg` byte-identical to what `bench/results.json` renders and has nothing to say about `53` citations inside a *released* changelog section, where staying current is the defect rather than the feature. Nothing in the tree distinguishes a live document from a frozen one. D-143. |
...
| `17` | **NEW (Mandate III).** A gated throughput baseline for the governed hot path, published in machine-independent counts with wall-clock unarmed (R17, R18) | **partly met** — one entry point, two real corpora, and the counts re-derived by the recorder | `governed_perf.*`, `10` run ids; `bench/run_governed_perf.py`, `28` tests; `docs/EVALUATION.md`. Tokenizer passes, catalog lookups split by origin, both memo hit rates, provenance records, `_prepare` calls and total Python calls are all gated; wall-clock is fenced with the machine named and never armed | The other three entry points. This decomposes `expand_identifier` alone — `is_compliant` and `to_physical_name` have none, `extract`, `disambiguate` and generation have none, and **memory is not measured at all**. **The seventh sweep re-derived this and the baseline deepened without widening**: `record_costs` decomposes what one provenance record costs into four constituents on four arms, `construction_ab` carries every work count on both sides of the switch, `identity` is the R19 comparison, and `memo` and `threads` add hit rates, catalog lookups, records constructed and a free-threaded census (D-100, D-101). **And the round that could most cheaply have closed one more entry point instead changed `is_compliant` and `to_physical_name` and measured both in a scratch script that is not committed** (D-099). It is still one entry point. **Re-derived at the eighth sweep and it is STILL one entry point**, on a round that added `17` benchmark run ids. Every one of them carries `elapsed_seconds` and `docs_per_second` and **not one carries a work count**, so R17's own criterion gained volume and no coverage; `docs_per_second` is documents divided by wall-clock and is a machine's number, not the library's. D-111 |
| 18 | **NEW (Mandate III).** Every optimisation is proven behaviour-identical byte for byte, forced on and forced off, over the full corpus, by a gate (R19) | **partly met — the rule now has three instances, one of them has a gate, and that gate has never seen a real schema in CI.** ~~not met, not started — and no obligation arose, because nothing was optimised~~ | **The obligation arose three times.** The governed memo has `tools/gate_memo_identity.py`, registered as `gates.memo_identity` with a `lint` step in `ci.yml`: `25` comparisons, `0` mismatches, and **the mutation that serves one caller another caller's answer fired on neither real corpus** until a generated corpus with case variants was built for it, so the gate had been green over four full-corpus runs on a class it could not see. The record-construction change is proven over `2,245,111` records by `repr` **and** `to_json` on four arms with a control that moves one `entry_id`, and that is a **runner mode a person invokes**, not a gate — no `tools/gate_record_identity.py` exists. The naming-contract change is proven over `857,517` records by a **scratch script that is not committed**. D-100, D-101, D-099 | A gate for each of the other two, and a CI job that fetches `data/` — the one registered gate runs against a generated fixture corpus in CI because the real corpora are fetched and never committed. **The rule's own test — that it has evidence it can fire — is now passed and it was passed by an accident**: the memo gate's fourth mutation was inert on both real corpora and only a purpose-built one revealed it. **At the eighth sweep a fourth full-corpus identity proof arrived and no fourth gate did.** A2's default path was proved byte-identical over `4,260` documents at three profiles and `10,625` pairs, digest against digest with a control perturbation that moves it, **twice by two parties with two independent serialisations** — and both are scratch scripts that are not committed. A2 is not an optimisation, so the obligation count stays at three; what rose is the count proved by an uncommitted script, from one to two. `tools/gate_record_identity.py` still does not exist. D-111 **At the ninth sweep the largest byte-identity result this project has ever produced was produced by a script that is not committed, which is this row's existing defect for a third time.** The package split was proved byte-identical over `3,619,227` of `3,619,228` snapshot records, provenance fields included — and `git ls-files` matches no snapshot harness anywhere in the tree, so no second party can re-run it and the sixth cold read could not adjudicate it either. **Two parties now unable, against D-110's one.** The split is not an optimisation, so the obligation count stays at three; what rose is the count proved by an uncommitted script, from two to three. D-120. **At the tenth sweep the rule caught a real defect for the first time in its history, and the arm that caught it was one of five.** D3's R19 pass over the deferred span route found `3` of `20,000` fixture records whose token is not a substring of its own identifier — `_rejoin_digit_tokens` makes `AMT_1_MM` yield `1MM` — so a `(start, end)` representation cannot address them. **It was `0` on both real corpora, because both run an empty catalog, which is every published governed arm**: the representation would have passed R19 everywhere this project measures and dropped a character on the first caller with a real vocabulary. A control probe that moves one `entry_id` ships beside it and must fire, and one test breaks the materialiser and requires mismatches — **the rule's own test, that it has evidence it can fire, is now passed on purpose rather than by accident.** D2's `selective.identity` adds a fifth instance (`3,882` records over `1,252` documents, three comparisons at `0` differences, plus `3,846` over the `gate=` path) and reports that **two of its three arms could not have failed** and the third caught a harness defect. Still `partly met`: no `tools/gate_record_identity.py` exists, no CI job fetches `data/`, and the `3,619,227`-record result still comes out of a script `git ls-files` does not match. D-128, D-129. |
| 19 | **NEW (Mandate III).** W11 — whether `extract()` may emit a short form with an absent or low-confidence long form — is decided | **not met, not started** | [`docs/notes/w11-emission-model.md`](notes/w11-emission-model.md) opens *"Status: scoping note. No decision, no behaviour change, nothing shipped."* It scopes the question, costs it and gives it a failure mode. Nothing in this phase touched it. **RE-DERIVED AT THE EIGHTH SWEEP AGAINST A ROUND WHOSE BRIEF FORECAST THIS ROW WOULD CLOSE. It does not.** A2 shipped as `acronymkit.nlp.propagation.propagate()`, and `grep -rn propagation src/acronymkit/` returns one hit outside that module — the lazy-submodule tuple. No engine call site, no `Config` field, no CLI verb; `extract()` is byte-identical over `4,260` documents; `ExtractionResult` carries no occurrences; and every propagated occurrence carries the definition A2 committed to, so **no long form is absent — it is elsewhere in the document.** A2 answers the adjacent question and makes this one cheaper. It does not take it | A product decision, taken out loud in a record. It bears on criterion `10`: answering it *yes* is the only route by which a span corpus could adjudicate the flagship claim. **And the cost of not taking it is now visible rather than hypothetical**: a shipped module reaches the occurrences W11 is about, through a door `extract()` does not open, so a caller who wants them must import a second module and the emission model is decided by omission. D-111, D-117 |
| 20 | **NEW (Mandate III).** The catalog-gap report is shipped **and has been run by a stranger** | **half met, and the half that matters is not** | [`docs/SOURCING.md`](SOURCING.md) ships with an owner, three dated actions, a `2026-11-23` expiry and a stranger-runnable kit. Its own log reads **approaches made: `0`** | One organisation saying yes. This is `U-0` with a delivery date attached, and it is the same people problem — no amount of work inside this repository moves it. **At the seventh sweep the shipped half got much stronger and the half that matters did not move at all.** `acronymkit governed-gap` ships — `src/acronymkit/catalog/gap.py`, `bench/run_catalog_gap.py`, `40` tests, `7` run ids, the only governed command where `--dictionary` is optional — so "shipped" now means a program rather than a document with a kit in it. **Approaches made is still `0`.** And the report carries its own caveat: on the only corpus it has been run against, `15` of its top `20` ranked tokens are single characters and `9` carry no letter, the filter that fixes that is off by default, and turning it on more than doubles the share of the gap with no work item at all. D-103 |
...
**Seventh sweep: what the two swaps mean, and one thing this page should stop doing.** Criterion `15`
closed and criterion `7` opened, so `11` of `20` is the same number describing a different tree. Read
the pair together rather than the total. `15` closed because a rule with no instances was built before
its first instance existed, which is the only order in which R16 could ever have worked — the script,
the gate and the CI step went in, and the figures went in afterwards and through them. `7` opened
because a criterion that had been carried as met for four sweeps was re-derived for the first time and
two live instances fell out, one of them shipped for the whole of that period.

**The lesson is about carrying, and it lands on this page.** Nine of twenty rested on somebody else's
answer at the sixth sweep; ten do now. Every one of those is a verdict that might be `7`. The page
already says which rows are carried, in the verdict column, and this is the first sweep where that
disclosure paid out. **A row carried for five sweeps is not evidence; it is a habit with a citation
attached**, and `2`, `5`, `6`, `8` and `12` are now in that group.
...
| `acronymkit.catalog` | `governed_gold.sec_xbrl.*`, `governed_gold.socrata.*` | held out, measured-before-declared |
...
```
one prose bullet inserted into README.md, then `python tools/check_claims.py`
  -- the first two rows as first measured; the rest re-run after the arming
     vocabulary was widened, on the same file, one sentence at a time
  ...
  ------------------------------------------------------------------- vocabulary widened here
  "Median latency for a governed expansion fell to 41 microseconds ..."   rc=1, README.md named
  "Extraction accuracy reached 99.94 % ..."   <- control, same line       rc=1, README.md named
  "The mean duration of one governed lookup was 41.37 ..."               rc=1, README.md named
  "Median latencies ... fell to 41.37 this quarter"                      rc=0, not named
  "A full governed sweep took 41.37 seconds"                             rc=0, not named
  "... is 41.37x faster than the baseline"                              rc=0, not named
  "... holds 41.37 KB of resident state"                                rc=0, not named
```
...
was never armed and never seen**, so both sentences were false — and **both were corrected at
`3173126`** to say *a performance claim that the gate can recognise*.

**And the hedge has now been paid for rather than only written.** Keeping the hole open so that six
documents describing it stayed accurate is circular debt, and this criterion is the one that says
so: a check that cannot fail is not done. `latency` and `duration` are metric keywords and the
spelled-out sub-second units are units, the rows above were **re-run** rather than having their
digits edited, and the four `rc=0` rows underneath are the residue — published, because a closure
reported as total is the same defect one level up. The hedge stays in both sentences, because it is
still true: the vocabulary is a list, and a list is finished by somebody.

**And *six* was wrong — the population is seven, corrected at the eighth sweep.** `docs/SOURCING.md`
carries the same demonstrated battery and the same present-tense sentence saying the gate cannot see a
latency claim, was in nobody's list, and still says it seven lines beneath an `rc=0` row for the
mutation that now reds. The sixth cold read found it by grepping the tree; no rotation could have. **Nothing re-runs a published mutation
battery when the gate it documents changes**, which is the class rather than the instance, and it is
the reason this criterion's own evidence blocks are worth distrusting. `docs/DECISIONS.md` D-112.
...
**Tenth sweep: the fenced command output above is five rounds stale, and this section is the reason
that matters rather than a place to hide it.** It prints `deferred 201`, `5 rounds` and
*"moved 12"*; the live figures are `deferred 185`, `11 rounds` and `moved 4`.

```
python tools/check_claims.py -- command output, re-run for the tenth sweep
  value-matched ratchet:  64 of 64 budgeted across 3 file(s)
  deferred ratchet:      185 of 185 budgeted across 10 file(s)
  ledger trajectory: 11 rounds | M3-PD2 (the first walk over all ten files)
                     moved 4 (citation 4, deletion 0, fencing 0, other 0)
                     0 of them out of docs/DECISIONS.md
                     deferred 185, value-matched 64
                     quota 12 per round, record-file floor 12
```

**Eleven observations: `54`, `31`, `18`, `12`, `12`, `0`, `0`, `0`, `0`, `4`.** Four consecutive
zeros on four consecutive waivers, then a payment of `4`. **And the reason the fifth waiver stands is
now arithmetic rather than a walk's verdict**: the quota is a floor of `12` per round on a payable
population measured at `4` across all ten files, `0` of it in the file the record floor is written
against. That is a different statement from *the residue is terminal* and it needs a different fix —
a floor the ledger can supply, or D-109's per-file `closed` disposition. **A waiver that must be
written every round carries no information, and this is the fifth.** D-136.
...
70.15<!--claim:governed_perf.pre_memo_socrata_empty.stage_provenance_pct:.2f--> % of the Socrata call and
