...
if TYPE_CHECKING:  # pragma: no cover - annotations only
    # THE ONE IMPORT THAT WOULD MAKE ``core`` NOT A LEAF, AND IT IS DEFERRED
    # RATHER THAN REMOVED. Nothing here constructs, subclasses or
    # ``isinstance``-checks a ``DisambiguationResult``: every use below is an
    # annotation over duck-typed attribute access (``.candidates``,
    # ``.score``, ``.expansion``). Same pattern, same reason, as
    # ``scoring.py``'s ``Lexicon``/``CharNGramModel`` imports -- see
    # docs/ARCHITECTURE.md, "Design decisions worth knowing".
