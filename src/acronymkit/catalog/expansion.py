...
:class:`~acronymkit.catalog.dictionary.GovernedDictionary` with an audit record
wrapped around them, and the only judgement anywhere in the path is
:func:`~acronymkit.catalog.tokenizer.split_identifier` deciding where one token
...
:mod:`~acronymkit.catalog.tokenizer` accounts for, or reported on
:attr:`~acronymkit.catalog.models.IdentifierExpansion.unaccounted`; and
...
from ..core.exceptions import ConfigurationError, LexiconError
...
#: not one: a :class:`~acronymkit.catalog.policy.NamingPolicy` is frozen, so one
...
#:
#: :mod:`acronymkit.catalog.compliance` and :mod:`acronymkit.catalog.naming`
...
def _prepare(
    ...
    :meth:`~acronymkit.catalog.dictionary.GovernedDictionary.with_custom` — so
...
def _empty_expansion(raw: str) -> TokenExpansion:
    ...
    :attr:`~acronymkit.catalog.models.TokenExpansion.long` is documented as
    ``""`` for empty input.
...
def _expand(token: str, dictionary: GovernedDictionary, policy: NamingPolicy) -> TokenExpansion:
    ...
    :mod:`~acronymkit.catalog.dictionary` module docstring for what makes that
    ...
    * a **passthrough is remembered in a different map**. It is not an answer the
      ...
      :class:`~acronymkit.catalog.dictionary._Memo` says why the split is by
      ...
      docstring of :mod:`~acronymkit.catalog.dictionary`.
...
def _rejoin_digit_tokens(
    ...
    :func:`~acronymkit.catalog.tokenizer.split_identifier` splits at every
...
def expand_token(
    ...
    *,
    ...
    Example:
        >>> from acronymkit.catalog import GovernedDictionary, expand_token
...
def expand_identifier(
    ...
    *,
    ...
    three :class:`~acronymkit.catalog.models.TokenExpansion` records each
    ...
    **A qualified name keeps its qualifier.** ``.`` is an ordinary separator, so
    ...
    :func:`~acronymkit.catalog.tokenizer.strip_qualifier` yourself when the
