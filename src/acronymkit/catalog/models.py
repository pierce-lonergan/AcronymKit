...
#: :mod:`~acronymkit.catalog.expansion` that reach these builders, ``beat``,
...
class GovernedEntry(_FrozenModel):
    ...
    * how far the catalog stands behind it — :attr:`confidence`,
    ...
    Example:
        >>> from acronymkit.catalog import EntryKind, ExpansionSource, GovernedEntry
        >>> entry = GovernedEntry(
        ...     token="TXN",
...
def _new_token_expansion(
    ...
    :mod:`~acronymkit.catalog.expansion` meet it by construction -- two pass the
...
class IdentifierExpansion(_FrozenModel):
    ...
    :mod:`~acronymkit.catalog.tokenizer` names, or listed in
...
def _new_identifier_expansion(
    ...
    already tuples; :func:`~acronymkit.catalog.expansion.expand_identifier`
    meets it because the first is a ``tuple`` comprehension and the second comes
    from :class:`~acronymkit.catalog.tokenizer.IdentifierParts`, every return
