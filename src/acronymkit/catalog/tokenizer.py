...
:func:`~acronymkit.catalog.compliance.normalize` rebuilds a name out of.
...
``acronymkit.nlp.tokenizer`` does apply NFKC. It applies it to build a *matching
...
class IdentifierParts(NamedTuple):
    ...
    :mod:`acronymkit.catalog.models`: this module is imported by every verb in
...
def _closes_an_ordinal(identifier: str, classes: list[int], position: int) -> bool:
    ...
    :func:`~acronymkit.catalog.compliance.normalize` cannot rebuild a name out
