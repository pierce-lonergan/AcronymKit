...
collision with no pin is settled by :func:`~acronymkit.catalog.scoring.rank_candidates`,
...
field of the frozen :class:`~acronymkit.catalog.policy.NamingPolicy` and so
...
dictionary)``: :func:`~acronymkit.catalog.expansion._title_case` reads the
...
frozen models. It is declared once, in :mod:`acronymkit.catalog.models`, and
...
#: :func:`~acronymkit.catalog.scoring.rank_candidates` rather than by a person.
#:
...
class _Memo(NamedTuple):
    ...
    :func:`~acronymkit.catalog.expansion._passthrough` before there is a record,
    ...
    Attributes:
        ...
        expanded: Surface token to its :class:`~acronymkit.catalog.models.TokenExpansion`,
            for tokens the vocabulary answered for. Filled by
            :mod:`acronymkit.catalog.expansion`, which owns the shape of that
        ...
        passed: Surface token to the passthrough
            :class:`~acronymkit.catalog.models.TokenExpansion` for a token the
            vocabulary does not answer for. Keyed by caller input; bounded by
            :data:`_MEMO_LIMIT`.
        identifiers: Whole identifier to its
            :class:`~acronymkit.catalog.models.IdentifierExpansion`. Keyed by
...
def _longest_long_form_words(*indexes: Mapping[str, GovernedEntry]) -> int:
    ...
    :mod:`acronymkit.catalog.naming` matches longest-first: at each position it
...
def _governed_choice(entry: GovernedEntry, token: str) -> str:
    ...
    :func:`~acronymkit.catalog.scoring.rank_candidates` when it is an
    unresolved collision, and its ``canonical`` otherwise. Deliberately
    independent of :class:`~acronymkit.catalog.policy.NamingPolicy`: this is
...
class GovernedDictionary:
    ...
    Example:
        >>> from acronymkit.catalog import EntryKind, ExpansionSource
        ...
        *,
        ...
        Raises:
            ValueError: If ``entries`` yields something that is not a
                :class:`~acronymkit.catalog.models.GovernedEntry`; the
    ...
    def from_long_to_short(
        ...
        * **several long forms** — a collision, and nobody has ruled on it, so
          :func:`~acronymkit.catalog.scoring.rank_candidates` chooses and the
    ...
    def resolve(
        ...
        lives; :class:`~acronymkit.catalog.models.TokenExpansion` has no
    ...
    def class_word_for(self, token: Optional[str]) -> Optional[str]:
        ...
        :class:`~acronymkit.catalog.models.IdentifierExpansion` reads it from
    ...
    def longest_long_form_words(self) -> int:
        ...
        :func:`~acronymkit.catalog.naming.to_physical_name` can bound its scan
    ...
    def _memo(self, policy: NamingPolicy) -> _Memo:
        ...
        Also called from :mod:`acronymkit.catalog.expansion`, which fills the
