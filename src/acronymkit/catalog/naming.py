...
:func:`~acronymkit.catalog.tokenizer.split_identifier_parts` — the same splitter
...
* **Two tokens claim one long form.** A catalog that carries both the approved
  ...
  :func:`~acronymkit.catalog.compliance.normalize` is for.
...
:data:`~acronymkit.catalog.tokenizer.ACCOUNTED_SEPARATORS` covers, so no word of
...
:attr:`~acronymkit.catalog.models.IdentifierExpansion.unaccounted` since it was
written. They are now on
:attr:`~acronymkit.catalog.models.PhysicalName.unaccounted`, and the only edit
that took was calling the splitter's lossless form.

**This verb reports rather than refuses, and the split from
:func:`~acronymkit.catalog.compliance.normalize` — which refuses — is a
...
:attr:`~acronymkit.catalog.models.PhysicalName.truncated` is written ``False``
...
:func:`~acronymkit.catalog.compliance.is_compliant` with
...
put back — is imported from :mod:`~acronymkit.catalog.expansion`, where the
...
:mod:`~acronymkit.catalog.compliance` imports it from here, so the class word
...
class _Rendered(NamedTuple):
    ...
    :class:`~acronymkit.catalog.models.PhysicalToken` carries no confidence
...
def _render(
    ...
    **The scan is bounded by the catalog, not by the name.** It used to run from
    ...
    :attr:`~acronymkit.catalog.dictionary.GovernedDictionary.longest_long_form_words`,
    ...
    *,
    ...
    Args:
        logical: The logical name — "Transaction Identifier", "customer account
            ...
            :func:`~acronymkit.catalog.tokenizer.split_identifier_parts`, so any
        ...
        custom: A caller-supplied overlay layered for this call only, as a plain
            ...
            :class:`~acronymkit.catalog.models.GovernedEntry` values. Under
    ...
    Returns:
        A :class:`~acronymkit.catalog.models.PhysicalName`. ``truncated`` is
    ...
    Example:
        >>> from acronymkit.catalog import GovernedDictionary, to_physical_name
