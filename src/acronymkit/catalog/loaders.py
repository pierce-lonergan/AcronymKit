...
:class:`~acronymkit.catalog.dictionary.GovernedDictionary` already knows how to
...
:func:`~acronymkit.catalog.scoring.canonical_form_score` exists to settle. The
semantics of that inversion are
:meth:`~acronymkit.catalog.dictionary.GovernedDictionary.from_long_to_short`'s
...
:class:`~acronymkit.catalog.dictionary.GovernedDictionary` already documents for
...
def load_csv(
    ...
    *,
    ...
    semantics are
    :meth:`~acronymkit.catalog.dictionary.GovernedDictionary.from_mapping`'s,
...
def load_long_to_short_csv(
    ...
    *,
    ...
    Returns:
        ...
        by :func:`~acronymkit.catalog.scoring.canonical_form_score` and
...
def _minted_entry(token: str, candidates: tuple[str, ...], pin: Optional[str]) -> GovernedEntry:
    ...
    Returns:
        ...
        :func:`~acronymkit.catalog.scoring.rank_candidates` and reports
        ``SCORED`` at :data:`~acronymkit.catalog.dictionary.DERIVED_ENTRY_CONFIDENCE`,
...
def _bundle_documents(path: Path) -> tuple[dict[str, Any], dict[str, Path], str]:
    ...
    if path.is_dir():
        ...
        if not files:
            ...
            raise LexiconError(
                ...
                f"aliases listed in acronymkit.catalog.loaders.BUNDLE_FILES) and found: "
                f"{present}."
            )
    ...
    if not documents:
        ...
        raise LexiconError(
            ...
            f"acronymkit.catalog.loaders.BUNDLE_FILES and found: {listed}."
        )
