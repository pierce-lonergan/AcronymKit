...
:class:`~acronymkit.catalog.enums.ComplianceReasonCode`, a sentence for a
...
--------------------------------------------------
:attr:`~acronymkit.catalog.policy.NamingPolicy.unknown` says what
:mod:`~acronymkit.catalog.expansion` does with a token the vocabulary does not
...
:attr:`~acronymkit.catalog.models.IdentifierExpansion.unaccounted` the whole
time; the accounting existed and this module threw it away by calling
:func:`~acronymkit.catalog.tokenizer.split_identifier` where its lossless twin
was one word away.
...
* :func:`~acronymkit.catalog.naming.to_physical_name` returns a record and also
  reports, on :attr:`~acronymkit.catalog.models.PhysicalName.unaccounted`.
...
tokenise through :func:`~acronymkit.catalog.expansion._rejoin_digit_tokens`, so
...
:mod:`~acronymkit.catalog.tokenizer` refuses to, or declining to upper-case a
...
:meth:`~acronymkit.catalog.policy.NamingPolicy.strict_length` an over-long name
...
from ..core.exceptions import ConfigurationError, TokenizationError
...
def _prepare(
    ...
    Splits through
    :func:`~acronymkit.catalog.tokenizer.split_identifier_parts` rather than
    :func:`~acronymkit.catalog.tokenizer.split_identifier`, so the characters
...
def is_compliant(
    ...
    *,
    ...
    Returns:
        A :class:`~acronymkit.catalog.models.ComplianceResult`. ``compliant`` is
    ...
    Example:
        >>> from acronymkit.catalog import GovernedDictionary, is_compliant
...
def normalize(
    ...
    *can* report do: :func:`is_compliant` gives
    ...
    :func:`~acronymkit.catalog.tokenizer.split_identifier_parts` gives the
    ...
    Raises:
        ...
        TokenizationError: If any character of ``name``, or of the corrected name
            ...
            :data:`~acronymkit.catalog.tokenizer.ACCOUNTED_SEPARATORS` covers.
    ...
    Example:
        >>> from acronymkit.catalog import GovernedDictionary, normalize
        >>> catalog = GovernedDictionary.from_mapping(
        ...
        Traceback (most recent call last):
            ...
        acronymkit.core.exceptions.TokenizationError: normalize cannot correct ...
    """
