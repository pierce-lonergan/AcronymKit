...
(:meth:`~acronymkit.catalog.policy.NamingPolicy.neural_optin`) and may only
...
(:class:`~acronymkit.catalog.enums.ExpansionSource`), which catalog row
...
:meth:`~acronymkit.catalog.policy.NamingPolicy.frequency_baseline` is likewise
...
``import acronymkit.catalog`` binds no submodule: every name below is resolved
...
:class:`~acronymkit.catalog.enums.EntryKind` never pays for the Pydantic
...
if TYPE_CHECKING:
    ...
    from .gap import CatalogGap, GapToken, catalog_gap, label_words, render_gap, stream_key
    from .loaders import (
        BUNDLE_FILES,
        load_bundle,
        load_csv,
...
#: ``import acronymkit.catalog; acronymkit.catalog.tokenizer`` works without
