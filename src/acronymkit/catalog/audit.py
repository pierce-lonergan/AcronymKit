...
:func:`~acronymkit.catalog.expansion.expand_identifier` once per column and
...
:mod:`acronymkit.catalog.naming` — so an identifier carrying an unapproved
...
:class:`~acronymkit.catalog.models.IdentifierExpansion`, because a corpus of
...
class UnknownToken(_FrozenModel):
    ...
    same call :mod:`acronymkit.catalog.compliance` makes when it declines to
...
class FindingTally(_FrozenModel):
    ...
    :func:`~acronymkit.catalog.compliance.is_compliant` — a review of one name
...
class CatalogSuggestion(_FrozenModel):
    ...
    **This is not an expansion and must never be used as one.** Nothing on it
    ...
    :func:`~acronymkit.catalog.expansion.expand_token` or
    :func:`~acronymkit.catalog.naming.to_physical_name` as though the catalog
...
def _trip_policy(policy: NamingPolicy) -> NamingPolicy:
    ...
    :mod:`acronymkit.catalog.naming`, whose third condition is that the
...
def _governed_word(dictionary: GovernedDictionary, token: str) -> Optional[GovernedEntry]:
    ...
    3. the short form the row names must itself be approved, which is the same
       rule :func:`~acronymkit.catalog.compliance._approved_form` applies for
...
def audit_identifiers(
    ...
    *,
    ...
    Example:
        ...
        >>> from acronymkit.catalog.audit import audit_identifiers
...
def render_audit(audit: CorpusAudit, *, limit: Optional[int] = 20) -> str:
    ...
    Example:
        >>> from acronymkit.catalog import GovernedDictionary
        >>> from acronymkit.catalog.audit import audit_identifiers, render_audit
        >>> nds = GovernedDictionary.from_long_to_short({"Transaction": "TXN"})
